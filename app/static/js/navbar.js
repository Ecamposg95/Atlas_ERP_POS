 ...
 * Actualizado para Atlas Technologies
 */
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (sidebarContainer) {
        // Recuperar sesión guardada en el Login del index.html
        ...
        if (storedUser) {
            ...
            const menuOptions = [
                ...
                {
                    label: 'Corte de Caja',
                    url: '#', // Pendiente de implementar vista visual
            ...
            // Generar HTML de enlaces
            const linksHTML = allowedOptions.map(opt => {
                // Lógica simple para "activo"
                const isActive = window.location.pathname === opt.url || (opt.url !== '/' && window.location.href.includes(opt.url));
        ...
        footerContainer.innerHTML = `
            <footer class="h-8 bg-slate-100 dark:bg-slate-950 border-t border-gray-200 dark:border-slate-800 flex justify-between items-center px-6 text-[10px] text-slate-500 shrink-0 select-none transition-colors duration-300">
                <div class="flex items-center gap-2">
                    <span class="font-bold">Powered by Atlas Technologies</span>
                    <span class="hidden sm:inline text-slate-300 dark:text-slate-700">|</span>
                    <span class="hidden sm:inline">v2.1.0</span>
                </div>
                <div class="font-mono font-medium flex items-center gap-4">
                    <span id="global-clock-date" class="hidden sm:inline"></span>
                    <span id="global-clock-time" class="text-slate-800 dark:text-slate-200 font-bold"></span>
                </div>
            </footer>
        `;
